...
매 회차에 바로 따라 할 수 있는 HTML 가이드를 모으는 폴더입니다. `guides.json` 항목과 파일명의 날짜는 **스터디 수행일이 아니라 가이드 제작일**입니다. 가이드 목록의 단일 원본은 [`guides.json`](./guides.json)이며, GitHub Pages의 [`guides/`](https://gist-isaac-robotics.github.io/2026-pnp-study/guides/) 화면이 이 파일을 읽어 최신순으로 표시합니다.
...
이미 실습과 해설에 사용한 완료 회차 가이드는 당시 절차를 재현하는 보존본으로 유지할 수 있습니다. 이후 커리큘럼이 간소화되더라도 사실 오류가 없는 보존본을 소급해 다시 쓰지는 않으며, 차이는 `progress.md`와 현재 커리큘럼의 해당 회차에 기록합니다. 현재 필수 범위와 Gate는 항상 루트 [`curriculum.md`](../curriculum.md)를 따르고, 실제 수행 결과는 [`progress.md`](../progress.md)와 전문 문서를 기준으로 판단합니다.
...
1. 이 폴더에 제작일 기준 `YYYY-MM-DD-session-회차-주제.html` 형식으로 HTML 파일을 추가합니다. 회차에는 `1`, `0-1`, `6A`처럼 커리큘럼의 식별자를 그대로 사용합니다.
...
```json
{
  ...
  "path": "./YYYY-MM-DD-session-X-topic.html",
  "tags": ["Week 0", "태그"]
}
```
