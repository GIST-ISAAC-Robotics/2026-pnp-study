...
- **상태:** 완료
- **수행:** `progress.md`, `docs/setup/docker.md`, Session 0-3 실행 가이드를 대상으로 환경을 가리키는 표현과 본문 서술 시점을 교차 점검
- **결과:** 시각 동기화 문제의 발생 범위를 가리키는 표현이 문서마다 달라, `curriculum.md`와 `docs/setup/docker.md` §11.7이 이미 쓰던 **메인 PC**로 통일했다. Session 0-3 가이드 C-1의 도입 callout은 문서 전체가 쓰는 「사전 검증」 표기로 맞췄고, C의 검증 범위 문단은 사전 검증과 회차 당일 채울 값의 구분이 드러나도록 다듬었다. 명령, 측정값, Gate 판정과 계약 내용은 변경하지 않았다.
- **문제:** 없음
- **결정:** 실행 환경은 `메인 PC`·`팀원 PC`로만 지칭하고, 가이드 본문은 1인칭 없이 서술한다. 이후 회차 기록과 가이드 작성 시 같은 규칙을 적용한다.
- **다음:** Session 0-3에서 핵심 위험 3종 spike 진행
- **증빙:** [Docker 개발 환경과 S0-2 결과](./docs/setup/docker.md), [Session 0-3 실행 가이드](./guides/2026-07-31-session-0-3-week0-spikes.html)
